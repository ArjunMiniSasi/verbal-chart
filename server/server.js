  ...
  let PLUMB_CACHE = null;

  function loadPlumbCache() {
    if (PLUMB_CACHE) return;
  
    if (!fs.existsSync(PLUMB_DATA_PATH)) {
      console.warn('⚠️ Plumb embeddings file not found');
      return;
    }
  
    console.log('📦 Loading Plumb embeddings into memory...');
    const raw = fs.readFileSync(PLUMB_DATA_PATH, 'utf8');
    ...
    console.log(`✅ Plumb cache loaded (${PLUMB_CACHE.chunks.length} chunks)`);
  }
  ...
  async function searchPlumbData(query, k = 5) {
    ...
    try {
      ...
      try {
        ...
        if (!PLUMB_CACHE) loadPlumbCache();
        const { chunks, embeddings } = PLUMB_CACHE;
  ...
  // ------------- generate-soap endpoint (FAST + MULTILINGUAL SOA) -------------
app.post('/api/generate-soap', async (req, res) => {
  try {
    ...
    console.log('📝 [FAST SOA MULTILINGUAL] Transcript length:', transcript.length);

    // 🔥 Trim transcript to control latency
      ...
      messages: [
        {
          role: 'system',
          content: `
You are a professional veterinary medical scribe.

The consultation transcript may be in ANY language 
(including Hindi, Marathi, Tamil, Telugu, Bengali, Kannada, Malayalam, Gujarati, Punjabi, or mixed regional speech).

Instructions:
- Internally understand and normalize the content.
- Convert all medical information into clear clinical English.
- Extract only facts explicitly mentioned.
- Do NOT hallucinate.
- Do NOT include Plan.
...
Required structure:
{
  ...
  "assessment": "..."
    ...
    const response = {
      ...
      soap: parsed,
      retrieval_debug: {
        message: 'RAG disabled - fast multilingual mode'
      }
    };
  ...
  *Follow-up:*
  ...
  Keep it very brief.`;
      ...
      // Use LLM to synthesize a treatment plan from Plumb references (optimized context size)
      const MAX_CHARS_PER_CHUNK = 800;  // reduce token load
      const MAX_REFERENCES = 4;         // no need to send 5-8 chunks

      const trimmedPlumb = plumbResults
      ...
      const plumbContext = trimmedPlumb;
